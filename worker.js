...
async function hashPassword(password, saltB64=randomToken(16), iterations=120000){
  const salt = Uint8Array.from(atob(saltB64.replace(/-/g,'+').replace(/_/g,'/').padEnd(Math.ceil(saltB64.length/4)*4,'=')), c=>c.charCodeAt(0));
...
async function notify(env,{userId,actorId,type,postId=null}){
  ...
  if(map[type] && ns[map[type]]===false) return;
...
async function getPublicProfile(env,viewer,username){
  ...
  const isOwn=viewer && Number(viewer.id)===Number(u.id);
...
const CHAT_THEMES={
  default:['#111111','#333333','#666666'],
  ocean:['#0077FF','#00B7FF','#7C3AED'],
  sunset:['#FF512F','#DD2476','#FFB347'],
  neon:['#00F5A0','#00D9F5','#7C3AED'],
  berry:['#7F00FF','#E100FF','#FF4D8D'],
  forest:['#064E3B','#10B981','#A3E635'],
  fire:['#FF0844','#FF4B2B','#FFB000'],
  sky:['#2563EB','#06B6D4','#A5F3FC'],
  candy:['#EC4899','#A855F7','#60A5FA'],
  royal:['#312E81','#7C3AED','#C026D3']
};
...
async function getChatTheme(env,chatId){
  ...
  const row=await env.DB.prepare(
    'SELECT theme FROM chat_themes WHERE chat_id=?'
  ).bind(Number(chatId)).first();

  const name=CHAT_THEMES[row?.theme]
    ? row.theme
    : 'default';
...
async function chatSummary(env,user,chatId){
  ...
  if(c.is_group){
    const theme=await getChatTheme(env,c.id);
...
async function createDirectChat(env,user,targetId){
  ...
  if(set.message_privacy==='followers'){
    ...
    if(!f)
      throw new HttpError(403,'Follow this user before messaging them');
  }
  ...
  if(existing)
    return await chatSummary(env,user,existing.id);
  ...
  return await chatSummary(env,user,id);
}

async function createGroup(env,user,body){
  const name=clean(body.name,60)||'New group';

  let ids=Array.isArray(body.user_ids)
    ? body.user_ids.map(Number).filter(Boolean)
    : [];

  ids=[...new Set(ids)]
    .filter(x=>x!==Number(user.id));

  if(ids.length>49)
    throw new HttpError(400,'A group can have at most 50 members');
  ...
  if(ids.length){
    ...
    if((rows.results||[]).length!==ids.length)
      throw new HttpError(400,'One or more users were not found');
  }

  const cr=await env.DB.prepare(
    'INSERT INTO chats (name,is_group) VALUES (?,1)'
  ).bind(name).run();
...
async function sendMessage(env,user,chatId,content){
  const member=await env.DB.prepare(`
    SELECT 1
    FROM chat_members
    WHERE chat_id=? AND user_id=?
...
async function sendResetEmail(env,email,code){
  ...
  const response=await fetch(
    'https://api.resend.com/emails',
    {
      ...
      body:JSON.stringify({
        ...
        to:[email],
        subject:'Your ZILNET password reset code',
        html:`
  ...
  if(!response.ok)
    throw new HttpError(502,'Could not send recovery email');
}
/* =========================================================
...
function adminCookie(token,maxAge=86400){
  return `zilnet_admin=${token}; Max-Age=${maxAge}; Path=/; HttpOnly; Secure; SameSite=Strict`;
}

function clearAdminCookie(){
  return 'zilnet_admin=; Max-Age=0; Path=/; HttpOnly; Secure; SameSite=Strict';
}

function githubCfg(env){
  ...
  return {
    token,
    owner:String(env.GITHUB_OWNER||'zilnet-main'),
    repo:String(env.GITHUB_REPO||'Zilnet'),
    branch:String(env.GITHUB_BRANCH||'main')
  };
}

function safeRepoPath(v){
  const p=String(v||'').replace(/^\/+/,'').trim();
...
async function github(env,apiPath,options={}){
  ...
  const r=await fetch(
    'https://api.github.com'+apiPath,
    {
      ...
      headers:{
        ...
        'User-Agent':'ZILNET-Developer-Console',
        ...(options.headers||{})
      }
    }
  );
  ...
  if(!r.ok){
    throw new HttpError(
      r.status,
      data?.message||`GitHub request failed (${r.status})`
    );
  }
...
async function githubGetFile(env,path){
  const c=githubCfg(env);
  const p=safeRepoPath(path);

  return github(
    env,
    `/repos/${encodeURIComponent(c.owner)}/${encodeURIComponent(c.repo)}/contents/${p.split('/').map(encodeURIComponent).join('/')}?ref=${encodeURIComponent(c.branch)}`
  );
}
...
async function githubPutFile(
  ...
  const body={
    message:clean(
      message||`ZILNET developer update: ${p}`,
      120
    ),
    content:b64Text(content),
    branch:c.branch
  };
  ...
  return github(
    env,
    `/repos/${encodeURIComponent(c.owner)}/${encodeURIComponent(c.repo)}/contents/${p.split('/').map(encodeURIComponent).join('/')}`,
    {
      method:'PUT',
      headers:{
        'content-type':'application/json'
...
/*
  Developer authentication uses a separate table.
  This means the developer session does not depend
  on an ordinary ZILNET user session.
*/

async function ensureDeveloperSessions(env){
  await env.DB.prepare(`
...
async function requireDeveloper(req,env){
  await ensureDeveloperSessions(env);

  const admin=getCookie(req,'zilnet_admin');
  ...
  /*
    Optional direct header authentication.
    Useful for trusted developer tools.
  */

  const supplied=
    req.headers.get('X-Developer-Password')||'';

  const configured=
    String(env.DEVELOPER_PASSWORD||'');
  ...
  /*
    Optional developer-user authentication.
  */

  const user=await requireUser(req,env);

  const allowed=
    String(env.DEVELOPER_USER_ID||'');

  if(
    allowed &&
    String(user.id)===allowed
  ){
    return user;
  }
...
async function adminFollowAll(
  env,
  username,
  remove=false
){
  const u=await userByUsername(
    env,
    clean(username,30).replace(/^@/,'')
  );
  ...
  return {
    ok:true,
    target:u.username,
    added:Number(r.meta?.changes||0)
  };
}

async function adminLikeAll(
  env,
  postId,
  remove=false
){
...
async function adminRename(
  ...
  const oldP=safeRepoPath(oldPath);
  const newP=safeRepoPath(newPath);

  if(oldP===newP)
    throw new HttpError(
      400,
      'Choose different file names'
    );
  ...
  /*
    ...
    This protects files from accidental replacement.
  */
  ...
  if(existing){
    throw new HttpError(
      409,
      `Destination already exists: ${newP}`
    );
  }

  const created=await githubPutFile(
    env,
    newP,
    unb64Text(f.content),
    null,
  ...
  try{
    ...
    throw new HttpError(
      502,
      `Created ${newP}, but could not remove ${oldP}: ${e.message}`
    );
  }
  ...
  return {
    ...
    old_path:oldP,
    new_path:newP,
    commit:created.commit?.sha||null
  };
}

async function adminSwap(
  env,
  first='index.html',
  second='indexopen.html'
){
  const a=safeRepoPath(first);
  const b=safeRepoPath(second);

  if(a===b)
    throw new HttpError(
      400,
      'Choose two different file names'
    );

  const fa=await githubGetFile(env,a);
  const fb=await githubGetFile(env,b);

  const ac=unb64Text(fa.content);
  const bc=unb64Text(fb.content);

  const temp=
    `.zilnet-swap-${Date.now()}-${randomToken(5)}.tmp`;
  ...
  */
  ...
  try{
    ...
    await githubDeleteFile(
      ...
      temp,
      ft.sha,
      'ZILNET swap: remove temporary file'
    );
  }catch{}
  ...
  return {
    ...
    first:a,
    second:b,
    note:'Files swapped through a temporary repository file.'
  };
}
  ...
  */
  ...
  if(
    ...
    return ok({
      ...
      github_configured:
        !!env.GITHUB_TOKEN,
      github_owner:
        String(env.GITHUB_OWNER||'zilnet-main'),
      github_repo:
        String(env.GITHUB_REPO||'Zilnet'),
      github_branch:
        String(env.GITHUB_BRANCH||'main')
    });
  }

  if(
    path==='/api/admin/follow-all' &&
    method==='POST'
  ){
    await requireDeveloper(
      req,
  ...
  if(
    ...
    return ok({
      ok:true,
      path:pathValue,
      commit:r.commit?.sha||null
    });
  }

  if(
    path==='/api/admin/file/backup' &&
    method==='POST'
  ){
    await requireDeveloper(
      req,
    ...
    const p=
      await env.DB.prepare(`
        SELECT
          ...
          (SELECT COUNT(*)
           FROM likes
           WHERE post_id=p.id) likes_count,

          (SELECT COUNT(*)
           FROM comments
           WHERE post_id=p.id) comments_count,
