...
async function ensureChatThemes(
  env
) {
  await env.DB.prepare(`
...
/*
  Your existing posts table may have been created before
  media_url and media_type existed.

  This safely adds only missing columns.
  Existing posts/data are preserved.
*/

async function ensurePostColumns(env) {
...
async function route(
  ...
  if (
    ...
    /*
      password_hash already contains the PBKDF2 salt.
    */
  ...
  /*
    ...
    /api/profile?username=someone
      -> requested public user

    This prevents the frontend from accidentally getting
    the current user's profile when inspecting someone else.
  */
  ...
  if (
    ...
    /*
      IMPORTANT:
      Make the old posts table compatible before inserting.
    */
